"""
This code is originally from the linear_operator package:
https://github.com/cornellius-gp/linear_operator/blob/main/linear_operator/operators/permutation_linear_operator.py

The implementation in the package does not allow to type cast the PermutationLinearOperator.
Because of time constraints, we patch it locally here for now.
"""
...
class PermutationLinearOperator(AbstractPermutationLinearOperator):
    ...
    def __init__(
        ...
        super().__init__(perm, inv_perm, validate_args=validate_args, dtype=dtype)
    ...
    def _transpose_nonbatch(self: Float[LinearOperator, "*batch M N"]) -> Float[LinearOperator, "*batch N M"]:
        return PermutationLinearOperator(perm=self.inv_perm, inv_perm=self.perm, validate_args=False, dtype=self._dtype)
    ...
    def to(self, *args, **kwargs) -> Float[LinearOperator, "*batch M N"]:
        """
        ...
        perm = self.perm.to(device=device)
        inv_perm = self.inv_perm.to(device=device)
        return PermutationLinearOperator(perm, inv_perm, False, dtype=dtype)
...
class TransposePermutationLinearOperator(AbstractPermutationLinearOperator):
    ...
    def __init__(self, m: int, dtype = None):
        if m < 1:
